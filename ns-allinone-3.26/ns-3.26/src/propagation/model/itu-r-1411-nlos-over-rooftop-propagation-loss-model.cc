...
ItuR1411NlosOverRooftopPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  ...
  double hb = (a->GetPosition ().z+1 > b->GetPosition ().z+1 ? a->GetPosition ().z+1 : b->GetPosition ().z+1);
  double hm = (a->GetPosition ().z+1 < b->GetPosition ().z+1 ? a->GetPosition ().z+1 : b->GetPosition ().z+1);
